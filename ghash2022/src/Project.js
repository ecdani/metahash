class Project {
    ...
    findContributors = (contributors) => {
      ...
      this.skillsNeeded.forEach(skillNeeded => {
        const contributor = contributors.find((contributor) => contributor.skills.find(s => contributor.free && s.name === skillNeeded.name && skillNeeded.level <= s.level));
        if (!contributor) {
          return;
        }
      ...
      //       return neededSkill.name === skill.name && neededSkill.level <= skill.level
      //     })) {
